...
def build_routes(app: Flask, llm: LlmExecutor) -> None:
    ...
    def chat_completions_endpoint():
        ...
        completion = llm.completion(prompt, int(body['max_tokens']), parse_temp(float(body['temperature'])), stops=body['stop'], repetition_penalty=body['frequency_penalty'])
    ...
    def completions_endpoint():
        body = request.get_json(force=True)
        completion = llm.completion(body['prompt'], int(body['max_tokens']), parse_temp(float(body['temperature'])), stops=body['stop'], repetition_penalty=body['frequency_penalty'])
