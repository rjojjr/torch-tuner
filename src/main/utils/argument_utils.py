...
def _build_program_argument_parser(title: str, description: str) -> ArgumentParser:
    ...
    parser.add_argument('-a', '--lora-alpha', type=int, help="LoRA Alpha value(determines LorA Scale[scale = alpha/r])(default: 16)", default=16)
    ...
    parser.add_argument('-lrst', '--lr-scheduler-type', default="linear", help="Learning rate scheduler type(determines the learning rate decrease as training progresses[helps stabilize tuning and prevent over-fitting])(ONLY applies to AdamW optimizers)(default: linear)")
